/* H750DEV Board HAL - based on TX16SMK3 */
/*
 * Copyright (C) EdgeTX
 *
 * Based on code named
...
TIM12:	BACKLIGHT_TIMER (TIM12_CH1 PWM on PH6)
TIM14:	MS_TIMER
TIM15:  PDM
TIM16:
...
// PLL1: HSE=25MHz / M=5 * N=192 / P=2 = 480MHz sysclk (VOS0 overdrive required)
// HCLK = sysclk / AHB_DIV_2 = 240MHz
// APB1 = APB2 = HCLK / APB_DIV_2 = 120MHz  (timer clk x2 = 240MHz)
...
// PG.07 = TOUCH_SDA (software I2C) — cannot also be KEY_PAGEUP (would cause stuck key)
// KEYS_GPIO_REG_PAGEUP / KEYS_GPIO_PIN_PAGEUP intentionally not defined
...
// PG.03 = TOUCH_SCL (software I2C) — cannot also be KEY_EXIT (would cause stuck key)
// KEYS_GPIO_REG_EXIT / KEYS_GPIO_PIN_EXIT intentionally not defined
// PI.15 = LTDC_R0; reassigned to PI3 (P2-25)
#define KEYS_GPIO_REG_TELE            GPIOI
#define KEYS_GPIO_PIN_TELE            LL_GPIO_PIN_3  // PI.03 (P2-25)
...
// PH.15 = LTDC_G4; reassigned to PH12 (P2-38)
#define TRIMS_GPIO_REG_LSD          GPIOH
#define TRIMS_GPIO_PIN_LSD          LL_GPIO_PIN_12  // PH.12 (P2-38)

#define TRIMS_GPIO_REG_LSU          GPIOH
#define TRIMS_GPIO_PIN_LSU          LL_GPIO_PIN_11  // PH.11
...
// PG.10 = LTDC_G3; reassigned to PI0 (P2-19)
#define IO_RESET_GPIO GPIO_PIN(GPIOI, 0)  // PI.00 (P2-19)
...
// Power
// Dev board: no power button, USB insertion provides power directly.
// PH7 (unused GPIO, not connected on dev board) is used as dummy PWR_SWITCH_GPIO:
// pwrInit() sets GPIO_IN_PU -> reads HIGH -> !gpio_read()=false -> pwrPressed() returns false.
// NOTE: Do NOT use PA10 here — it is also LCD_RESET_GPIO (conflict).
#define PWR_SWITCH_GPIO             GPIO_PIN(GPIOH, 7)
// PWR_ON_GPIO not defined     -> pwrOn()/pwrOff() are no-ops (no latching circuit)
...
// USB_GPIO_VBUS not connected on dev board
...
// SPI LCD (FPC 10P connector)
// SPI5: SCK=PK0, MOSI=PJ10, NSS(CS)=PH5, DC=PJ11
#define LCD_SPI                         SPI5
#define LCD_SPI_GPIO_AF                 GPIO_AF5
#define LCD_SPI_SCK_GPIO                GPIO_PIN(GPIOK, 0)   // PK.00
#define LCD_SPI_MOSI_GPIO               GPIO_PIN(GPIOJ, 10)  // PJ.10
#define LCD_SPI_CS_GPIO                 GPIO_PIN(GPIOH, 5)   // PH.05
#define LCD_SPI_DC_GPIO                 GPIO_PIN(GPIOJ, 11)  // PJ.11

// PJ.12 = LTDC_B0; LCD panel reset reassigned to PA10 (P2-20, was BACKLIGHT now free)
#define LCD_RESET_GPIO                  GPIOA
#define LCD_RESET_GPIO_PIN              LL_GPIO_PIN_10  // PA.10 (P2-20)

// Backlight
// PH6 -> TIM12_CH1 (AF2); TIM12 is shared with MIXER_SCHEDULER_TIMER (update IRQ)
// The CH1 PWM output and the update IRQ can coexist on TIM12
#define BACKLIGHT_GPIO                  GPIO_PIN(GPIOH, 6)  // PH.06 / TIM12_CH1
#define BACKLIGHT_TIMER                 TIM12
...
#define QSPI_FLASH_SIZE                 0x800000   // W25Q64: 64Mbit = 8MB
...
#define LED_GREEN_GPIO                    GPIO_PIN(GPIOI, 5)   // PI.05 (P2-31)
#define LED_BLUE_GPIO                     GPIO_PIN(GPIOI, 9)   // PI.09 (P3-31)
...
//   TRAINER_OUT_GPIO was GPIO_PIN(GPIOI, 5) which conflicts with LED_GREEN_GPIO
#define TRAINER_RCC_AHB1Periph          (RCC_AHB1Periph_GPIOA)

// TRAINER_IN_GPIO  (GPIOI, 6)  — not connected
// TRAINER_OUT_GPIO (GPIOI, 5)  — not connected (was conflicting with LED_GREEN)
...
// TRAINER_DETECT_GPIO (GPIOI, 2) — not connected on dev board
...
// Touch — GT911 on software I2C (per schematic: SCLK=PG3, SDA=PG7)
#define TOUCH_I2C_SCL_GPIO              GPIO_PIN(GPIOG, 3)  // PG.3  (TOUCH_SCLK per schematic)
#define TOUCH_I2C_SDA_GPIO              GPIO_PIN(GPIOG, 7)  // PG.7  (TOUCH_SDA  per schematic)
#define TOUCH_INT_GPIO                  GPIO_PIN(GPIOI, 11) // PI.11 (TOUCH_INT  per schematic)
#define TOUCH_RST_GPIO                  GPIO_PIN(GPIOI, 10) // PI.10 (TOUCH_RST  per schematic)

// TOUCH_INT uses EXTI line 11 -> covered by USE_EXTI15_10_IRQ defined below for IMU
...
// Rotary Encoder: A=PI7 (P3-45), B=PI4 (P3-47) - PJ8 was LTDC_G1, replaced with PI4
...
#define ROTARY_ENCODER_EXTI_PORT_B      LL_SYSCFG_EXTI_PORTI
...
// NOTE: TIM12 is shared with BACKLIGHT_TIMER (TIM12_CH1 PWM on PH6).
// mixerSchedulerStart() resets CCER/CCMR1/PSC/ARR and would destroy the
// backlight PWM configuration, so use TIM4 (free on this board) instead.
#define MIXER_SCHEDULER_TIMER                TIM4
#define MIXER_SCHEDULER_TIMER_FREQ           (PERI1_FREQUENCY * TIMER_MULT_APB1)
#define MIXER_SCHEDULER_TIMER_IRQn           TIM4_IRQn
#define MIXER_SCHEDULER_TIMER_IRQHandler     TIM4_IRQHandler
