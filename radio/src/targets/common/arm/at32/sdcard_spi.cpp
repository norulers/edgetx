 ...
 *
 * AT32 version of the SD-card SPI protocol driver.
 * Ported from targets/common/arm/stm32/sdcard_spi.cpp using the AT32 SPI API.
 */
...
static inline void _send_dummy_byte(const at32_spi_t* spi)
{
  at32_spi_transfer_byte(spi, SD_CARD_DUMMY_BYTE);
}

static bool _wait_for_not_busy(const at32_spi_t* spi, uint32_t retry_us)
{
  uint32_t timeout = timersGetUsTick();

  do {
    uint8_t read_byte = at32_spi_transfer_byte(spi, SD_CARD_DUMMY_BYTE);
    if (read_byte == 0xFF) {
      return true;
    }
    if ((read_byte & 0x0F) != 0x00) {
      // shift by some bits???
      at32_spi_unselect(spi);
...
static inline uint16_t _transfer_bytes(const at32_spi_t* spi, const uint8_t* out,
                                       uint8_t* in, uint16_t length)
{
  return at32_spi_transfer_bytes(spi, out, in, length);
}

static void _flush_block(const at32_spi_t* spi)
{
...
static inline uint8_t _wait_for_r1(const at32_spi_t* spi, uint32_t retry_us)
{
  uint32_t timeout = timersGetUsTick();
  uint8_t r1;

  do {
    r1 = at32_spi_transfer_byte(spi, SD_CARD_DUMMY_BYTE);
...
static inline bool _wait_for_token(const at32_spi_t* spi, uint8_t token, uint32_t retry_us)
{
  uint32_t timeout = timersGetUsTick();
  uint8_t read_byte;

  do {
    read_byte = at32_spi_transfer_byte(spi, SD_CARD_DUMMY_BYTE);
    if (read_byte == token) {
      return true;
    }
...
static uint8_t sdcard_spi_send_cmd(const at32_spi_t* spi, uint8_t sd_cmd_idx,
...
static uint8_t sdcard_spi_send_acmd(const at32_spi_t* spi, uint8_t sd_cmd_idx,
                                    uint32_t argument, uint32_t retry_us)
{
  uint32_t timeout = timersGetUsTick();
  uint8_t r1_resu;
...
static sd_rw_response_t _read_cid(const at32_spi_t* spi, sdcard_info_t* card);
static sd_rw_response_t _read_csd(const at32_spi_t* spi, sdcard_info_t* card);

static sd_init_fsm_state_t _init_sd_fsm_step(const at32_spi_t* spi,
                                             sdcard_info_t* card,
                                             sd_init_fsm_state_t state)
{
  ...
  case SD_INIT_START:
    TRACE("SD_INIT_START");
    at32_spi_init(spi, AT32_SPI_DATAWIDTH_8BIT);
    at32_spi_set_max_baudrate(spi, SD_SPI_CLK_400K);
  ...
  case SD_INIT_SPI_POWER_SEQ:
    ...
    for (int i = 0; i < 10; i++) {
      at32_spi_transfer_byte(spi, 0xff);
    }
    {
  ...
  case SD_INIT_SET_MAX_SPI_SPEED:
    ...
    at32_spi_set_max_baudrate(spi, SD_SPI_CLK_MAX);
...
int sdcard_spi_init(const at32_spi_t* spi, sdcard_info_t* card)
{
...
static sd_rw_response_t _read_data_packet(const at32_spi_t* spi, uint8_t token,
                                          uint8_t *data, uint16_t size)
{
  ...
  if (at32_spi_dma_receive_bytes(spi, data, size) == size) {
...
static sd_rw_response_t _write_data_packet(const at32_spi_t* spi, uint8_t token,
                                           const uint8_t *data, uint16_t size)
{
  ...
  at32_spi_transfer_byte(spi, token);

  if (at32_spi_dma_transmit_bytes(spi, data, size) != size) {
  ...
  do {
    uint8_t data_response = at32_spi_transfer_byte(spi, SD_CARD_DUMMY_BYTE);
    ...
    if (cmd_idx == SD_CMD_25) {
      if (!_wait_for_not_busy(spi, SD_WAIT_FOR_NOT_BUSY_US)) {
        *state = SD_RW_TIMEOUT;
        ...
        at32_spi_transfer_byte(spi, SD_DATA_TOKEN_CMD_25_STOP);
      }
    }
  }
  else {
