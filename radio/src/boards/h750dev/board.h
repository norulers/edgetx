...
#define FLASHSIZE                       0x800000   // W25Q64: 64Mbit = 8MB
...
// BOOTLOADER_KEYS not used: boardBLStartCondition() in board.cpp uses direct GPIO (PH12+PH13)
...
// PWR_BUTTON_PRESS is already defined via CMakeLists.txt (PWR_BUTTON=PRESS).
// Explicitly defining here too for clarity and to prevent regression if cmake
// default changes. This selects the state-machine pwrCheck() which returns
// e_power_on by default (enabling menusTask while-loop to run perMain()).
...
// Secondary SPI LCD (top panel) driver
...
// Bootloader UART debug (USART1 PA9, 115200 8N1)
#ifdef __cplusplus
extern "C" {
#endif
