...
extern "C" void SDRAM_GPIOConfig(void)
{
  ...
  */
  ...
  gpio_init_af(GPIO_PIN(GPIOH, 2), GPIO_AF_FMC, GPIO_PIN_SPEED_VERY_HIGH);  // SDCKE0
  gpio_init_af(GPIO_PIN(GPIOH, 3), GPIO_AF_FMC, GPIO_PIN_SPEED_VERY_HIGH);  // SDNE0 (CS)
...
extern "C" void SDRAM_InitSequence(void)
{
  ...
  FMC_SDRAMCommandStructure.CommandTarget = FMC_SDRAM_CMD_TARGET_BANK1;  // Bank1: SDNE0=PH3
  ...
  // Actual clock chain: HSE=25MHz, PLL1(M=5,N=192,P=2)=480MHz, AHB/2=240MHz, SDRAM PERIOD_3=80MHz (12.5ns)
  // W9825G6KH: 8192 rows, tREF=64ms -> 64ms/8192 = 7.8125us per row
  // RefreshCount = 7.8125us * 80MHz - 20 = 625 - 20 = 605
  FMC_SDRAM_ProgramRefreshRate(FMC_Bank5_6_R, 605);
}
...
extern "C" void SDRAM_Init(void)
{
  ...
  // SDRAM clock = 80MHz (12.5ns period): HCLK=240MHz / PERIOD_3
  FMC_SDRAMTimingInitStructure.LoadToActiveDelay    = 2;  // tMRD = 2 clocks min
  FMC_SDRAMTimingInitStructure.ExitSelfRefreshDelay = 6;  // tXSR = 70ns / 12.5ns = 5.6 -> 6
  FMC_SDRAMTimingInitStructure.SelfRefreshTime      = 4;  // tRAS = 42ns / 12.5ns = 3.4 -> 4
  FMC_SDRAMTimingInitStructure.RowCycleDelay        = 5;  // tRC  = 60ns / 12.5ns = 4.8 -> 5
  FMC_SDRAMTimingInitStructure.WriteRecoveryTime    = 2;  // tWR  = 1CLK + 7.5ns = 12.5+7.5 -> 2
  FMC_SDRAMTimingInitStructure.RPDelay              = 2;  // tRP  = 18ns / 12.5ns = 1.4 -> 2
  FMC_SDRAMTimingInitStructure.RCDDelay             = 2;  // tRCD = 18ns / 12.5ns = 1.4 -> 2
